...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Moved outside the <nav> element to prevent backdrop-blur containment issues */}
      {isOpen && (
        <div className="fixed inset-0 bg-slate-950 z-40 flex flex-col items-center justify-center pt-20 gap-8 animate-in slide-in-from-top-10 fade-in duration-300">
          {navItems.map((item) => (
            <a 
              key={item.label} 
              href={`#${item.id}`} 
